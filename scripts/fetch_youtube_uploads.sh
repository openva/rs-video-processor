...
# Note: cookies are read directly from Firefox for each download via
# --cookies-from-browser firefox. Firefox cookie storage does not require
# keychain access, so no prompts are expected.
